...
class AvatarS3FlowTest(TestCase):
    ...
    def test_complete_avatar_flow(self):
        ...
        print(
            f"[Backend] Fazendo upload do avatar para: {profile.avatar.storage.__class__.__name__}")
        profile.avatar.save(
            f"test_avatar_{self.user.id}.jpg", self.avatar_file, save=True)
        ...
        avatar_view_url = reverse('account:avatar_view', kwargs={
        ...
        if "s3" in target_s3_url or "shortly-storage-prod" in target_s3_url:
            ...
            try:
                ...
                if s3_response.status_code == 200:
                    print("[SUCCESS] Imagem acessível pelo cliente! (HTTP 200)")
                    print(
                        f"[SUCCESS] Content-Type: {s3_response.headers.get('Content-Type')}")
                else:
                    print(
                        f"[FAILURE] S3 negou acesso: {s3_response.status_code}")
                ...
                self.assertIn(
                    "image/", s3_response.headers.get("Content-Type", "").lower())
            except Exception as e:
                print(f"[ERROR] Falha de rede ao conectar ao S3: {str(e)}")
                raise e
        else:
            print(
                f"[WARNING] O storage atual ({profile.avatar.storage.__class__.__name__}) não parece ser S3. URL: {target_s3_url}")
